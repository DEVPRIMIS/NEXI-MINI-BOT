...
if (autobio) {
  prim.updateProfileStatus(` ɴᴇxɪ ᴍɪɴɪ ʙᴏᴛ is Online✅ Runtime ${runtime(process.uptime())}`)
    .catch(err => console.error("Error updating status:", err));
}

if (m.sender.startsWith("92") && global.anti92 === true) {
  return prim.updateBlockStatus(m.sender, "block");
}
...
if (m.message.extendedTextMessage?.contextInfo?.mentionedJid?.includes(global.owner + "@s.whatsapp.net")) {
  if (!m.quoted) {
    ...
    setTimeout(() => {
      prim.sendMessage(m.key.remoteJid, { delete: m.key });
    }, 2000);
  }
}
    ...
    return prim.sendMessage(m.chat, {
      ...
      contextInfo: {
        ...
        externalAdReply: {
          ...
          title: "https://files.catbox.moe/jskkjl.jpg",
          renderLargerThumbnail: false,
          title: "｢ OWNER OFFLINE ｣",
