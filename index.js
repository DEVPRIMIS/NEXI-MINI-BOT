...
let phoneNumber = "50940938083"
let owner = JSON.parse(fs.readFileSync('./data/owner.json'))

global.botname = "ɴᴇxɪ ᴍɪɴɪ ʙᴏᴛ"
global.themeemoji = "•"
...
async function startconn() {
    ...
    if (pairingCode && !conn.authState.creds.registered) {
        ...
        if (!!global.phoneNumber) {
            ...
            phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Please type your WhatsApp number 😍\nFormat: 50941xxxxxx (without + or spaces) : `)))
        }
        ...
        if (!pn('+' + phoneNumber).isValid()) {
            console.log(chalk.red('Invalid phone number. Please enter your full international number (e.g., 50941658706 for Haïti, etc.) without + or spaces.'));
            process.exit(1);
        }
...
┗❐═⭔════════⭔═❐`,
            ...
            console.log(chalk.magenta(`\n${global.themeemoji || '•'} YT CHANNEL: DS PRIMIS`))
            console.log(chalk.magenta(`${global.themeemoji || '•'} GITHUB: DEV PRIMIS`))
            console.log(chalk.magenta(`${global.themeemoji || '•'} WA NUMBER: ${owner}`))
            console.log(chalk.magenta(`${global.themeemoji || '•'} CREDIT: DS PRIMIS`))
